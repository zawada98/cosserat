...
def add_cosserat_tube(root_node, p, x_offset=0.0,
    ...
    # ---- Visual model --------------------------------------------------------
    # modified: visual setup extracted to add_tube_visual(frame_node, p, color)
    # was: inline MeshTopology + visMO + RigidMapping + OglModel + IdentityMapping
    # The visual chain is now built explicitly in createScene() so the caller
    # has direct control over the rendering color (and can choose to skip
    # visualization entirely, e.g. for headless benchmarking runs).

    # frame_node returned so createScene can wire FramesMO into SSIM/BCM
    # AND attach the visual sub-graph via add_tube_visual().
...
class CTRController(Sofa.Core.Controller):
    ...
    def __init__(self,
        ...
        # rigid base is rewritten every step from these values.
        self._t1_pos_m     = 0.0
        self._t2_pos_m     = 0.0
    ...
    def onAnimateBeginEvent(self, event):
        ...
        # state where it is.  The pose is written every step regardless to
        # keep the rigid bases pinned at the initial offset.  This mirrors the
        # previous controller's behaviour: rigid bases stay clamped while the
        # strain DOFs of the inner tube relax from conform-to-outer.
        self._set_pose(self.t1_base_mo, self._t1_pos_m, self._t1_angle_rad,
                       x0=0.0)
...
def createScene(root_node):
    ...
    # ---- Visual models ------------------------------------------------------
    # modified: visualization is now an explicit step in createScene().
    # Pass color=... to override the default 'color' field of the param dict;
    # pass color=None (or omit) to use that default.  Any of these forms work:
    #     color=[r, g, b, a]          -- list/tuple of 4 floats
    #     color='r g b a'             -- whitespace-separated string
    #     color=None                  -- fall back to p['color']
    #
    # Example overrides (uncomment to test):
    #     add_tube_visual(t1_frame_node, T1_PARAMS, color=[0.0, 0.7, 1.0, 1.0])
    #     add_tube_visual(t2_frame_node, T2_PARAMS, color='1.0 0.2 0.2 1.0')
    ...
    # ---- GUI bridge ---------------------------------------------------------
    # [MODIFIED] Construct the Tkinter GUI bridge BEFORE the controller and the
    # InitializationMonitor.  The bridge:
    #   - spawns its own daemon thread for tk.mainloop()
    #   - exposes snapshot() / consume_dt_request() / signal_init_complete()
    #   - holds a reference to root_node so the Initialize button can flip
    #     animate=True at click time
    # The controller stashes this object (self.gui = gui_bridge) so Python's
    # GC cannot collect it during the simulation.
    #
    # dt parameters:
    #   init_dt              -- the dt SOFA actually runs at during init.
    #                           MUST equal root_node.dt above (1e-4 here).
    #   default_control_dt   -- the value the dt Spinbox SUGGESTS once
    #                           control phase begins.  It is NOT auto-applied
    #                           (an earlier auto-bump version crashed the
    #                           solver -- see CTRGuiBridge.signal_init_complete
    #                           for the post-mortem).  The user types/edits
    #                           the value and clicks Apply to actually change
    #                           root_node.dt.
    ...
    # ---- Controller (GUI-driven) --------------------------------------------
    # [MODIFIED] The controller no longer carries a hard-coded actuation
    # script.  All slider/knob values, dt requests and phase transitions
    # arrive via gui_bridge.snapshot() at the start of every step.
    # See CTRController docstring for the full phase state machine.
    ...
    # Reads bcm.distances and contactMO_gap.position once equilibrium is
    # detected, then pops up a one-shot 2D plot of normal gap vs curvilinear
    # abscissa of Pc_B along Tube_3.  See init_monitor.py for the class.
    #
    # Tube_3 frame curvilinear abscissae are read back from the
    # DiscreteCosseratMapping object that we just built (it stores the same
    # frm_curv_abs vector we passed at construction).  Reading from the
    # mapping object avoids modifying add_cosserat_tube's return signature.
    ...
    intersection_node.addObject(InitializationMonitor(
        ...
        # ---- Tunables (defaults shown; tune for your scene) ----
        # With dt = 1e-5 these correspond to:
        #   warmup = 5  ms ,  quiet = 1 ms , velocity threshold = 1 mm/s
        ...
        # [MODIFIED] When equilibrium is detected, the monitor calls this
        # callback BEFORE rendering its PNG plot.  signal_init_complete()
        # flips bridge.phase to 'control' and sets the init_complete flag
        # the GUI poll loop is waiting on; the dt is left untouched (an
        # earlier version auto-bumped it here and crashed the solver --
        # see CTRGuiBridge.signal_init_complete docstring for the full
        # post-mortem).  The user changes dt manually via the Spinbox.
    ...
    intersection_node.addObject(LiveContactMonitor(
        ...
        t2_frame_curv_abs=t2_curv_abs_frames,
        bridge=gui_bridge,
        # At dt = 1e-5 s, every_n_steps=20 -> push every 0.2 ms sim time;
        # GUI repaints at 50 ms wall-clock cadence.  Tune up if you see
        # the SOFA loop spending visible time inside push_contact_profile;
        # tune down if the plot looks choppy.
        every_n_steps=20,
    ))
    # -------------------------------------------------------------------------

    # ---- Pause until the user clicks "Initialize" ---------------------------
    # [MODIFIED] The scene is fully built but the simulation does NOT step
    # until the user clicks the Initialize button on the Tk window, which
    # writes root_node.animate = True from the GUI thread.  This way the
    # inner tube sits visibly in its conform-to-outer initial state at t=0
    # rather than starting to relax immediately on scene load.
    #
    # If runSofa is configured to auto-play scenes (or the user clicks Play
    # in the runSofa toolbar before clicking Initialize), the relaxation
    # will start anyway.  The phase machine in CTRController/InitMonitor is
    # robust to that case: pressing Initialize after the fact merely marks
    # the phase transition; the eventual transition to 'control' still
    # happens when equilibrium is detected.
