...
from comparison_between_ulc_and_cpuc.init_monitoring import InitializationMonitor
...
def _geometry_arc(p, init_strategy, outer_params):                        # modified
    L      = p['length']
    r_crv  = p['crv_radius']
    nb_sec = p['nb_sections']
    nb_frm = p['nb_frames']
    ...
    if init_strategy == 'natural':
        ...
        if outer_params is None or 'y0' not in outer_params:              # modified
            raise ValueError(
...
class CTRController(Sofa.Core.Controller):
    ...
    def __init__(self,
        ...
        # Inner: filled on init->control transition (NOT at construction).  # modified
        self._t2_rest_pose = None                                            # modified
