...
class CTRGuiBridgeStraightOuter:
    ...
    def push_contact_surface_profile(self, step, abscissae_m, angles_rad, gaps_m):
        """
        Called by LiveContactMonitor when cross-section contact coordinates
        are available.  angles_rad are measured in the inner tube's local
        cross-section as atan2(local_z, local_y).
        """
        with self._lock:
    ...
    def _build_plot_panel(self):
        ...
        force_ax.set_xlabel(
            r'Curvilinear abscissa of $P_{c,B}$ along inner tube [mm]')
    ...
    def _poll_surface_plot(self):
        try:
            ...
            if angles is None or len(angles) == 0:
                ...
                s_mm = abscissae * 1e3
                radii = np.ones_like(angles, dtype=float)
                self._surface_scatter.set_offsets(np.c_[angles, radii])
            ...
            print(f"[CTRGui5DOF] _poll_surface_plot error: {e!r}")
