...
class CTRGuiBridge:
    ...
    def __init__(self,
                 ...
                 default_trans_step_um=0.005,      # default slider chase speed [µm/step]
                 trans_step_min_um=0.001,          # Spinbox lower bound  [µm/step]
        ...
        self._shared = {
            ...
            'contact_profile': {
                ...
                'forces_n':  None,    # numpy array, newtons (or None)
                'dirty':     False,   # set True on push, False on consume
            },
            'contact_surface_profile': {
                'step':       0,
                'abscissae':  None,
                'angles_rad': None,
                'gaps_m':     None,
        ...
        self._force_line    = None
        self._force_scatter = None

        self._surface_window  = None
        self._surface_canvas  = None
        self._surface_ax      = None
        self._surface_scatter = None

        self._curvature_window = None
        self._curvature_canvas = None
    ...
    def push_contact_profile(self, step, abscissae_m, gaps_m, forces_n=None):
        """
        ...
        with self._lock:
            self._shared['contact_profile'] = {
                ...
                'forces_n':  (None if forces_n is None else
                              np.asarray(forces_n, dtype=float).copy()),
                'dirty':     True,
            }

    def push_contact_surface_profile(self, step, abscissae_m, angles_rad, gaps_m):
        """Optional cross-section contact profile pushed by LiveContactMonitor."""
        with self._lock:
            self._shared['contact_surface_profile'] = {
                'step':       int(step),
                'abscissae':  np.asarray(abscissae_m, dtype=float).copy(),
    ...
    def push_curvature_profile(self, step, abscissae_m, kappa, kappa_rest,
                               protrusion_m):
        """Live exposed-inner-tube curvature profile pushed by SOFA."""
        with self._lock:
            self._shared['curvature_profile'] = {
                'step':         int(step),
                'abscissae':    np.asarray(abscissae_m, dtype=float).copy(),
                'kappa':        np.asarray(kappa, dtype=float).copy(),
                'kappa_rest':   np.asarray(kappa_rest, dtype=float).copy(),
    ...
    def _build_widgets(self):
        ...
        ttk.Label(dt_frame,
                 text=f'(default {self._default_trans_step_um:.3g} µm/step; '
                      f'range [{self._trans_step_min_um:.3g}, '
                      f'{self._trans_step_max_um:.3g}] µm/step; live)'
    ...
    def _build_plot_panel(self):
        ...
        fig = Figure(figsize=(9.0, 6.2), dpi=100)
        ax = fig.add_subplot(211)
        force_ax = fig.add_subplot(212, sharex=ax)

        # Reference line at δn = 0 (the wall-to-wall touching threshold).
        ax.axhline(0.0, color='k', linewidth=0.8, alpha=0.7)
        force_ax.axhline(0.0, color='k', linewidth=0.8, alpha=0.7)
        ...
        force_ax.set_xlabel(
            r'Curvilinear abscissa of $P_{c,B}$ along Tube_3 [mm]')
        ax.set_ylabel(r'Normal gap $\delta_n$ [µm]')
        ax.set_title('Live contact profile  (waiting for data...)')
        ax.grid(True, alpha=0.3)
        force_ax.set_ylabel('Normal contact force [N]')
        force_ax.grid(True, alpha=0.3)

        (self._force_line,) = force_ax.plot(
            [], [], '-', color='gray', linewidth=0.8, alpha=0.6)
        self._force_scatter = force_ax.scatter(
            [], [], s=22, c=[], cmap='viridis',
            edgecolors='k', linewidths=0.4, zorder=3)
        fig.tight_layout()
    ...
    def _poll_plot(self):
        ...
        try:
            ...
            if abscissae is None or len(abscissae) == 0:
                ...
                self._force_line.set_data([], [])
                self._force_scatter.set_offsets(np.empty((0, 2)))
                self._force_scatter.set_array(np.empty(0))
                ...
                if forces_n is None:
                    valid_force = np.zeros_like(s_mm, dtype=bool)
                    finite_force = np.empty(0, dtype=float)
                else:
                    forces_n = np.asarray(forces_n, dtype=float)
                    valid_force = np.isfinite(forces_n)
                    finite_force = forces_n[valid_force]

                if finite_force.size == 0:
                    self._force_line.set_data([], [])
                    self._force_scatter.set_offsets(np.empty((0, 2)))
                    self._force_scatter.set_array(np.empty(0))
                    self._force_ax.set_title('Contact forces unavailable')
                else:
                    sf = s_mm[valid_force]
                    self._force_line.set_data(sf, finite_force)
                    self._force_scatter.set_offsets(np.c_[sf, finite_force])
                    self._force_scatter.set_array(finite_force)
                    fmax = max(1e-12, float(np.abs(finite_force).max()))
                    self._force_scatter.set_clim(0.0, fmax)
                    self._force_ax.set_title(
                        f'Contact force profile  '
                        f'(max = {float(finite_force.max()):.3g} N)')
    ...
    def _build_surface_plot_panel(self):
        win = tk.Toplevel(self._tk_root)
        win.title('CTR - Live contact cross-section')
        win.protocol('WM_DELETE_WINDOW', win.withdraw)
        self._surface_window = win

        fig = Figure(figsize=(5.5, 5.5), dpi=100)
        ax = fig.add_subplot(111, projection='polar')
        theta = np.linspace(0.0, 2.0 * np.pi, 256)
        ax.plot(theta, np.ones_like(theta), color='k', linewidth=0.8, alpha=0.5)
        ...
        ax.grid(True, alpha=0.3)
        ax.set_title('Live contact cross-section  (waiting for data...)')

        self._surface_scatter = ax.scatter(
            [], [], s=42, c=[], cmap='viridis',
            edgecolors='k', linewidths=0.4, zorder=3)
        cbar = fig.colorbar(
            self._surface_scatter, ax=ax, pad=0.10, shrink=0.75)
        cbar.set_label('Curvilinear abscissa [mm]')
        fig.tight_layout()
        ...
        self._surface_canvas = canvas
        self._surface_ax = ax

    def _poll_surface_plot(self):
        try:
            with self._lock:
                cp = self._shared['contact_surface_profile']
                if not cp['dirty']:
                    return
                step = cp['step']
                abscissae = cp['abscissae']
                angles = cp['angles_rad']
                gaps_m = cp['gaps_m']
                cp['dirty'] = False

            if angles is None or len(angles) == 0:
                self._surface_scatter.set_offsets(np.empty((0, 2)))
                self._surface_scatter.set_array(np.empty(0))
                self._surface_ax.set_title(
                    f'Live contact cross-section  (step {step}, K = 0)')
            else:
                s_mm = abscissae * 1e3
                self._surface_scatter.set_offsets(
                    np.c_[angles, np.ones_like(angles, dtype=float)])
                self._surface_scatter.set_array(s_mm)
                ...
                if abs(s_max - s_min) < 1e-12:
                    s_min -= 0.5
                    s_max += 0.5
                self._surface_scatter.set_clim(s_min, s_max)

                n_pen = int((gaps_m * 1e6 < 0.0).sum())
                self._surface_ax.set_title(
                    f'Live contact cross-section  (step {step}, '
                    f'K = {len(angles)}, penetrating = {n_pen})')

            self._surface_canvas.draw_idle()

        except Exception as e:
            print(f"[CTRGui] _poll_surface_plot error: {e!r}")

        finally:
            self._tk_root.after(
                self.PLOT_POLL_INTERVAL_MS, self._poll_surface_plot)

    def _build_curvature_plot_panel(self):
        win = tk.Toplevel(self._tk_root)
        win.title('CTR - Live exposed curvature')
        win.protocol('WM_DELETE_WINDOW', win.withdraw)
        self._curvature_window = win

        fig = Figure(figsize=(8.0, 4.8), dpi=100)
        ax = fig.add_subplot(111)
        (self._curvature_line,) = ax.plot(
            [], [], '-', color='#1f77b4', linewidth=1.6,
            label='actual curvature')
        (self._curvature_rest_line,) = ax.plot(
            [], [], '--', color='#d62728', linewidth=1.4,
            label='natural/rest curvature')
        ax.set_xlabel('Tube_3 curvilinear abscissa [mm]')
        ax.set_ylabel('Curvature kappa_z [1/mm]')
        ax.set_title('Exposed Tube_3 curvature  (waiting for protrusion...)')
        ax.grid(True, alpha=0.3)
        ax.legend(loc='best')
        fig.tight_layout()
        ...
        self._curvature_canvas = canvas
        self._curvature_ax = ax

    def _poll_curvature_plot(self):
        try:
            with self._lock:
                cp = self._shared['curvature_profile']
                if not cp['dirty']:
                    return
                step = cp['step']
                abscissae = cp['abscissae']
                kappa = cp['kappa']
                kappa_rest = cp['kappa_rest']
                protrusion_m = cp['protrusion_m']
                cp['dirty'] = False

            if abscissae is None or len(abscissae) == 0:
                self._curvature_line.set_data([], [])
                self._curvature_rest_line.set_data([], [])
                self._curvature_ax.set_title(
                    f'Exposed Tube_3 curvature  '
                    f'(step {step}, protrusion = {protrusion_m * 1e3:.1f} mm)')
            else:
                s_mm = np.asarray(abscissae, dtype=float) * 1e3
                kappa = np.asarray(kappa, dtype=float)
                kappa_rest = np.asarray(kappa_rest, dtype=float)

                self._curvature_line.set_data(s_mm, kappa)
                self._curvature_rest_line.set_data(s_mm, kappa_rest)
                self._curvature_ax.set_title(
                    f'Exposed Tube_3 curvature  '
                    f'(step {step}, protrusion = {protrusion_m * 1e3:.1f} mm)')

                self._curvature_ax.relim()
                self._curvature_ax.autoscale_view()

            self._curvature_canvas.draw_idle()

        except Exception as e:
            print(f"[CTRGui] _poll_curvature_plot error: {e!r}")
