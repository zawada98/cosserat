...
def compute_symmetric_arc_in_straight_beam(outer_beam, arc,
    ...
    """
    Set the straight beam length and arc angle for the intended placement.

    The outer beam is straight along world X and centered on y=z=0. The arc
    lies in the XY plane, is symmetric about the world Y axis, and has its
    midpoint on the beam centerline. Its two extremities are tangent to the
    lower inner wall of the outer beam, up to endpoint_clearance.
    """
    rin_outer = outer_beam['rin']
    rex_arc   = arc['rex']
    r_crv     = arc['crv_radius']

    if endpoint_clearance < 0.0:
        raise ValueError("endpoint_clearance must be non-negative.")

    endpoint_y_abs = rin_outer - rex_arc - endpoint_clearance
    if endpoint_y_abs <= 0.0:
        raise ValueError(
            "Arc cannot fit inside the outer tube: "
            f"rin_outer={rin_outer}, rex_arc={rex_arc}, "
            f"endpoint_clearance={endpoint_clearance}."
        )
    if endpoint_y_abs >= r_crv:
        raise ValueError(
            "Arc curvature radius is too small for the requested endpoint "
            f"offset: endpoint_y_abs={endpoint_y_abs}, r_crv={r_crv}."
        )

    phi_half = math.acos(1.0 - endpoint_y_abs / r_crv)
    ...
    print(
        "[Geometry] symmetric arc placement: "
