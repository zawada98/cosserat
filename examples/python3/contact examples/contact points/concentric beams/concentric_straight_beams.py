...
Beam 2 – PARALLEL to Beam 1, spring-driven at its base, settles as a whole.

Beam 2 starts GAP_Z mm directly above Beam 1.  During initialization the base
control point descends to the nested-contact clearance so the proximal end
settles with the rest of the beam.
...
Run with:
    runSofa concentric_straight_beams_driven_spring.py
"""
...
# initialization phase the base control point is lowered to this Z so the
# proximal frame settles with the rest of the inner beam instead of remaining
# suspended at the control point.
...
class CTRController(Sofa.Core.Controller):
    """
    GUI controller for the spring-driven scene.

    Unlike concentric_straight_beams_overwriting_DoFs.py, this controller
    never overwrites the inner beam's simulated RigidBaseMO.  It writes an
    external Rigid3d control point.  A RestShapeSpringsForceField on the
    inner base pulls the base toward that moving control point.
    """
    ...
    def onAnimateBeginEvent(self, event):
        ...
        if phase == 'control':
            ...
            self._write_control_point_pose(pose)
        else:
    ...
    def _settle_inner_control_z(self):
        pose = read_rigid_pose(self.t2_control_mo)
        pose[2] = self._step_toward(float(pose[2]),
                                    INNER_BEAM_SETTLE_Z,
                                    INNER_BEAM_SETTLE_STEP)
        self._write_control_point_pose(pose)

    def _write_control_point_pose(self, pose):
        """SOFA-thread write to the spring target, not to the simulated base."""
        write_rigid_pose(self.t2_control_mo, pose)
...
def createScene(root_node: Sofa.Core.Node):
    ...
    #   base control point so the full beam settles onto Beam 1. The base end
    #   (X = 0) follows a GUI-driven control point through a
    #   RestShapeSpringsForceField.  The controller does not overwrite
    #   RigidBaseMO directly.
    #
    beam2_control_mo = add_base_control_point(
        root_node,
        'Beam2',
    ...
    beam2_framesNode, beam2_frames, base_mo2 = add_cosserat_beam(
        ...
        beam_number    = 2,
        fully_fixed    = False,
        base_control_mo=beam2_control_mo,
        base_control_active_directions=[1, 1, 1, 1, 1, 1, 1],
    )
