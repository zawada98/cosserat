...
ContactPointsUnilateralConstraint::ContactPointsUnilateralConstraint()
    ...
    , d_normalContactImpulses(initData(&d_normalContactImpulses,
        "normalContactImpulses",
        "Solved normal Lagrange impulse for each active contact pair. Divide by dt for force."))
{
}
...
void ContactPointsUnilateralConstraint::rebuildContacts()
{
    ...
    d_activeContactPairIndices.setValue(sofa::type::vector<int>());
    d_normalContactImpulses.setValue(sofa::type::vector<Real>());
        ...
        const sofa::linearalgebra::BaseVector* lambda)
{
    ...
    sofa::type::vector<int> pairIds;
    sofa::type::vector<Real> normalImpulses;
    pairIds.reserve(m_contacts.size());
    normalImpulses.reserve(m_contacts.size());
    for (const Contact& con : m_contacts)
    {
        pairIds.push_back(con.k);
        normalImpulses.push_back(static_cast<Real>(lambda->element(con.cId)));
    }
